...
class PositionalEncoding(nn.Module):
    def __init__(self, L, C):
        ...
        self.position_embeddings = nn.Parameter(torch.randn(L, C))

    def forward(self, x, P):
        B, L_P, C = x.shape
        L = self.position_embeddings.shape[0]
        
        position_embeddings_expanded = self.position_embeddings.repeat_interleave(P, dim=0)
        
        if L_P != position_embeddings_expanded.shape[0]:
            raise ValueError("The expanded position embeddings do not match the sequence length.")
        
        x = x + position_embeddings_expanded.unsqueeze(0).expand(B, -1, -1)  # expand to batch size
        return x

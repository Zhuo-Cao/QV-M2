...
def batched_generalized_temporal_iou(batch_spans1, batch_spans2):
    # expect batch_spans1: (B, N, 2) and batch_spans2: (B, M, 2)
    B = batch_spans1.size(0)
    giou_list = []
    for i in range(B):
        giou = generalized_temporal_iou(batch_spans1[i], batch_spans2[i])
        giou_list.append(giou)
    return torch.stack(giou_list)  # (B, N, M)

def batched_temporal_iou(batch_spans1, batch_spans2):
    # expect batch_spans1: (B, N, 2) and batch_spans2: (B, M, 2)
    B = batch_spans1.size(0)
    iou_list = []
    for i in range(B):
        iou, _ = temporal_iou(batch_spans1[i], batch_spans2[i])
        iou_list.append(iou)
    return torch.stack(iou_list)  # (B, N, M)